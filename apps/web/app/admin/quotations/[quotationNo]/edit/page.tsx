...
const drinks: Array<{ id: DrinkId; label: string; hot: boolean }> = [
  { id: "americano", label: "Americano", hot: true }, { id: "latte", label: "Cafe Latte", hot: true },
  { id: "chocolate", label: "Dark Chocolate", hot: true }, { id: "lemonade", label: "Lemonade", hot: false }
];
...
export default function AdminQuotationEditPage() {
  ...
  useEffect(() => { loadQuotationByNo(params.quotationNo).then((quotation) => quotation && setData({ ...quotation, followUpStatus: quotation.followUpStatus ?? "NEW", status: quotation.status ?? "PENDING_APPROVAL" })).catch(() => setError("Unable to load quotation.")); }, [params.quotationNo]);
  ...
  return <main className="admin-page"><Card className="admin-card admin-edit-card">
    ...
    <div className="admin-form-grid">
      ...
      <section><h2>Follow-up</h2><label className="admin-field"><span>Status</span><select value={data.followUpStatus} onChange={(event)=>patch({followUpStatus:event.target.value as QuotationData["followUpStatus"]})}>{["NEW","CONTACTED","FOLLOW_UP","WON","LOST"].map(value=><option key={value}>{value}</option>)}</select></label><label className="admin-field"><span>Note</span><textarea rows={5} value={data.followUpNote ?? ""} onChange={(event)=>patch({followUpNote:event.target.value})} /></label></section>
    </div>
